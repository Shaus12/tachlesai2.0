...
const ChatArea = ({
  ...
  const getPlaceholderText = () => {
    if (isChatDisabled) {
      if (sourceCount === 0) {
        return t('notebook.chat.uploadToStart');
      } else {
        return t('notebook.chat.waitForProcessing');
      }
    }
    return t('notebook.chat.startTyping');
  };
  return <div className="flex-1 flex flex-col h-full overflow-hidden">
      {hasSource ? <div className="flex-1 flex flex-col h-full overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-200 flex-shrink-0">
            <div className="max-w-4xl mx-auto flex items-center justify-between">
              <h2 className="text-lg font-medium text-gray-900">{t('notebook.chat.title')}</h2>
              {shouldShowRefreshButton && <Button variant="ghost" size="sm" onClick={handleRefreshChat} disabled={isDeletingChatHistory || isChatDisabled} className="flex items-center space-x-2">
                  ...
                  <span>{isDeletingChatHistory ? t('notebook.chat.clearing') : t('notebook.chat.clearChat')}</span>
                </Button>}
            </div>
          </div>
                ...
                <div className="flex items-center space-x-4 mb-6">
                  ...
                  <div>
                    <h1 className="text-2xl font-medium text-gray-900">
                      {isGenerating ? t('notebook.generating') : notebook?.title || t('notebook.untitledNotebook')}
                    </h1>
                    <p className="text-sm text-gray-600">{sourceCount} {t('notebook.source', { count: sourceCount })}</p>
                  </div>
                </div>
                
                <div className="bg-gray-50 rounded-lg p-6 mb-6">
                  {isGenerating ? <div className="flex items-center space-x-2 text-gray-600">
                      <p>{t('notebook.aiAnalyzing')}</p>
                    </div> : <MarkdownRenderer content={notebook?.description || t('notebook.noDescription')} className="prose prose-gray max-w-none text-gray-700 leading-relaxed" />}
                </div>
              ...
              <div className="flex space-x-4">
                <div className="flex-1 relative">
                  ...
                  <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-sm text-gray-500">
                    {sourceCount} {t('notebook.source', { count: sourceCount })}
                  </div>
                </div>
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-8 overflow-hidden">
          <div className="text-center mb-8">
            ...
            <h2 className="text-xl font-medium text-gray-900 mb-4">{t('notebook.chat.addSourceToStart')}</h2>
            <Button onClick={() => setShowAddSourcesDialog(true)}>
              <Upload className="h-4 w-4 mr-2" />
              {t('notebook.chat.uploadSource')}
            </Button>
          </div>
          ...
          <div className="w-full max-w-2xl">
            <div className="flex space-x-4">
              <Input placeholder={t('notebook.chat.uploadToStart')} disabled className="flex-1" />
              <div className="flex items-center text-sm text-gray-500">
                0 {t('notebook.source', { count: 0 })}
              </div>
              <Button disabled>
      ...
      <div className="p-4 border-t border-gray-200 flex-shrink-0">
        <p className="text-center text-sm text-gray-500">{t('notebook.chat.disclaimer')}</p>
      </div>
