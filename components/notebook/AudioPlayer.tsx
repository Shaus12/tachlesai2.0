...
const AudioPlayer = ({ 
  ...
  useEffect(() => {
    ...
    const handleError = async (e: Event) => {
      ...
      if (retryCount < 2 && !autoRetryInProgress) {
        ...
        setAudioError(t('notebook.studio.audio.failedToLoad'));
  ...
  }, [onError, isExpired, retryCount, notebookId, onUrlRefresh, audioError, autoRetryInProgress, t]);
    ...
    if (isPlaying) {
      ...
      if (playPromise !== undefined) {
        playPromise.catch(error => {
          ...
          setAudioError(t('notebook.studio.audio.playbackFailed'));
        });
      }
    }
  ...
  const downloadAudio = async () => {
    ...
    try {
      ...
      toast({
        title: t('notebook.studio.audio.downloadStarted'),
        description: t('notebook.studio.audio.downloadStartedDesc'),
      });
    } catch (error) {
      console.error('Download failed:', error);
      toast({
        title: t('notebook.studio.audio.downloadFailed'),
        description: t('notebook.studio.audio.downloadFailedDesc'),
        variant: "destructive",
      });
    } finally {
  ...
  const deleteAudio = async () => {
    if (!notebookId) {
      toast({
        title: t('notebook.studio.audio.error'),
        description: t('notebook.studio.audio.cannotDelete'),
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: t('notebook.studio.audio.deleted'),
        description: t('notebook.studio.audio.deletedDesc'),
      });
      ...
      toast({
        title: t('notebook.studio.audio.deleteFailed'),
        description: t('notebook.studio.audio.deleteFailedDesc'),
        variant: "destructive",
      });
    } finally {
      setIsDeleting(false);
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={downloadAudio} disabled={isDownloading}>
              ...
              {isDownloading ? t('notebook.studio.audio.downloading') : t('notebook.studio.audio.download')}
            </DropdownMenuItem>
            <DropdownMenuItem 
              onClick={deleteAudio}
      ...
      {autoRetryInProgress && (
        <div className="flex items-center justify-between p-3 bg-blue-50 rounded-md border border-blue-200">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-blue-600">{t('notebook.studio.audio.refreshingAccess')}</span>
          </div>
        </div>
      )}
