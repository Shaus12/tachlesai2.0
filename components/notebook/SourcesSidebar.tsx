    ...
    <div className="w-full bg-gray-50 border-r border-gray-200 flex flex-col h-full overflow-hidden">
      <div className="p-4 border-b border-gray-200 flex-shrink-0">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-medium text-gray-900">{t('notebook.sources.title')}</h2>
        </div>
        
        <div className="flex space-x-2">
          <Button variant="outline" size="sm" className="flex-1" onClick={() => setShowAddSourcesDialog(true)}>
            <Plus className="h-4 w-4 mr-2" />
            {t('notebook.sources.add')}
          </Button>
        </div>
      </div>
        ...
        <div className="p-4">
          {isLoading ? (
            <div className="text-center py-8">
              <p className="text-sm text-gray-600">{t('notebook.sources.loading')}</p>
            </div>
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">{t('notebook.sources.emptyTitle')}</h3>
              <p className="text-sm text-gray-600 mb-4">{t('notebook.sources.emptyDesc')}</p>
            </div>
          )}
        </div>
      </ScrollArea>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('notebook.sources.deleteTitle', { name: selectedSource?.title })}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('notebook.sources.deleteDesc')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction 
              ...
              disabled={isDeleting}
            >
              {isDeleting ? t('notebook.sources.deleting') : t('notebook.sources.delete')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
