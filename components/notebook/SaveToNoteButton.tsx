...
const SaveToNoteButton = ({ content, notebookId, onSaved }: SaveToNoteButtonProps) => {
  ...
  const handleSaveToNote = () => {
    ...
    if (isAIResponse) {
      ...
      const firstSegmentText = content.segments[0]?.text || t('notebook.studio.notes.aiResponse');
  ...
  return (
    ...
    >
      ...
      <span className="text-xs">{isCreating ? t('notebook.studio.notes.saving') : t('notebook.studio.notes.saveToNote')}</span>
    </Button>
  );
};
