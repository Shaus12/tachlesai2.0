...
const NotebookGrid = () => {
  ...
  const sortedNotebooks = useMemo(() => {
    ...
    if (sortBy === t('dashboard.sort.recent')) {
      return sorted.sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
    } else if (sortBy === t('dashboard.sort.title')) {
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <Button className="bg-black hover:bg-gray-800 text-white rounded-full px-6" onClick={handleCreateNotebook} disabled={isCreating}>
          {isCreating ? t('dashboard.creating') : t('dashboard.createNew')}
        </Button>
        ...
        <div className="flex items-center space-x-4">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onClick={() => setSortBy(t('dashboard.sort.recent'))} className="flex items-center justify-between">
                {t('dashboard.sort.recent')}
                {sortBy === t('dashboard.sort.recent') && <Check className="h-4 w-4" />}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortBy(t('dashboard.sort.title'))} className="flex items-center justify-between">
                {t('dashboard.sort.title')}
                {sortBy === t('dashboard.sort.title') && <Check className="h-4 w-4" />}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
